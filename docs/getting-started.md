...
theorem ofNat_toNat : ∀ (a : Mod3), ofNat (toNat a) = a := by
  intro a
  cases a <;> rfl
```
...
4. Leaves `∀ a' b' : Nat, a' + b' = b' + a'` for `omega`
...
This says:
- **2a** (covariant): We have a map `toNat : Mod3 → Nat` with `toNat a = b → R a b`
...
theorem to_from : ∀ a, from_other (to_other a) = a := ...
...
theorem embed_project : ∀ a, project (embed a) = a := ...
...
theorem quotient_lift : ∀ a, quotient (lift a) = a := ...
...
**Solution**: tr_from_map expects `∀ a, f (g a) = a`. If you have the opposite direction, you need `tr_from_involution` or manual construction:

```lean
instance : Param42b α β := by
  tr_constructor
  exact (f · = ·)  -- R
...
**Congratulations!** You now know the basics of proof transfer with Trale. Happy proving! 🎉
